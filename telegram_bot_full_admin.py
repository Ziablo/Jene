...
"""
Bot Telegram avec Panel Admin Complet
Admin: @grandjd
...
ADMIN_ID = None  # Optionnel : Mettez votre user ID si vous le connaissez
...
def is_admin(update: Update) -> bool:
    """Vérifie si l'utilisateur est admin - VERSION AMÉLIORÉE"""
    user = update.effective_user
    
    # Debug info
    logger.info(f"🔍 Vérification admin:")
    logger.info(f"  - User ID: {user.id}")
    logger.info(f"  - Username: {user.username}")
    logger.info(f"  - First name: {user.first_name}")
    logger.info(f"  - Admin attendu: {ADMIN_USERNAME}")
    
    # Vérification par username (sans @)
    if user.username:
        username_lower = user.username.lower()
        admin_lower = ADMIN_USERNAME.lower()
        
        logger.info(f"  - Comparaison: '{username_lower}' == '{admin_lower}' ?")
        
        if username_lower == admin_lower:
            logger.info("  ✅ ADMIN DÉTECTÉ PAR USERNAME")
            return True
    
    # Vérification par ID si défini
    if ADMIN_ID and user.id == ADMIN_ID:
        logger.info("  ✅ ADMIN DÉTECTÉ PAR ID")
        return True
    
    logger.info("  ❌ PAS ADMIN")
    return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # VÉRIFICATION ADMIN
    if is_admin(update):
        logger.info(f"🔐 Affichage du panel admin pour @{user.username}")
        await show_admin_panel(update, context)
        return
    
    logger.info(f"👤 Affichage du message utilisateur normal pour @{user.username}")
...
async def debug_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Commande de debug pour voir vos infos"""
    user = update.effective_user
...
• Nom : {user.last_name or 'N/A'}

**Configuration admin :**
• Admin attendu : {ADMIN_USERNAME}
• Admin ID : {ADMIN_ID or 'Non défini'}

**Test :**
• Es-tu admin ? **{'✅ OUI' if is_admin(update) else '❌ NON'}**

**Si tu es @grandjd mais pas détecté :**
Envoie-moi ton user ID `{user.id}` pour que je l'ajoute dans le code.
"""
...
async def show_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_admin(update):
        logger.warning(f"⚠️ Tentative d'accès admin refusée pour {update.effective_user.username}")
...
def main() -> None:
    logger.info(f"🤖 Bot avec Panel Admin - VERSION DEBUG")
    logger.info(f"👤 Admin : @{ADMIN_USERNAME}")
    logger.info(f"🆔 Admin ID : {ADMIN_ID or 'Non défini'}")
    ...
    logger.info("✅ Bot démarré avec debug activé !")
    logger.info("💡 Utilisez /debug pour voir vos infos")
