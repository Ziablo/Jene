...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, menu_id: str) -> None:
    ...
    if menu_id not in menus:
        logger.error(f"❌ Menu '{menu_id}' introuvable")
        if query:
    ...
    logger.info(f"📋 Affichage menu '{menu_id}' avec {len(buttons_data)} boutons")
    ...
    # Si pas de boutons et que c'est le menu principal, ajouter un message d'info
    if not keyboard and menu_id == 'main_menu':
        message += "\n\n_Aucun bouton configuré. Utilisez /admin pour ajouter des boutons._"
    
    # Bouton retour pour les sous-menus
    if menu_id != 'main_menu':
        keyboard.append([InlineKeyboardButton("🔙 Retour", callback_data='main_menu')])
    
    # S'assurer qu'il y a au moins un bouton (même vide) pour éviter les erreurs
    if not keyboard:
        keyboard = [[InlineKeyboardButton("🔄 Recharger", callback_data='main_menu')]]
    ...
    if query:
        try:
            ...
            logger.info(f"✅ Menu '{menu_id}' affiché")
        except Exception as e:
            logger.error(f"❌ Erreur affichage menu: {e}")
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info(f"📍 Callback reçu : {data}")
    ...
    try:
        ...
        # User - autres menus
        elif data in bot_config.get("menus", {}):
            await query.answer()
            await show_menu(update, context, data)
        
        # User - réponses personnalisées
        else:
            logger.info(f"🔍 Recherche réponse pour : {data}")
            responses = bot_config.get("responses", {})
            if data in responses:
                ...
                await query.answer(f"❌ Action '{data}' non trouvée", show_alert=True)
                logger.warning(f"⚠️ Callback non géré : {data}")
